...
|-----------|--------------|-------|-----|
| **Bootstrap hub (automático)** | Webnu al abrir Pantallas | `POST /integrations/webnu/hub/bootstrap` | Crea/recupera cuenta TVPik + token sin intervención del usuario |
| **Webnu → TVPik (manual)** | Panel Webnu → Avanzado | `POST /admin/tvpik/connect` | Pegar token TVPik (fallback) |
| **TVPik → Webnu** | Workspace TVPik | `GET/POST /integrations/tvpik/connect` | OAuth: login Webnu desde app TVPik |

## Flujo del restaurante (hub en Webnu)

1. Plan con **Pantallas** (Plus o Pro + add-on) → menú **Pantallas** en el panel.
2. Al abrir `/admin/tvpik`, Webnu llama **bootstrap** → cuenta TVPik activa en segundo plano.
3. **Nueva pantalla** → nombre → pantalla creada en TVPik.
4. En la TV: app TVPik → código de 6 caracteres → **Emparejar TV** en Webnu.
5. Por cada pantalla: elegir carta, plantilla TV y **Publicar**.
6. Editar platos en **Mi carta** → republicación automática (cola de jobs).

No hace falta abrir tvpik.es para carta-en-TV. El reproductor HDMI (`?player=1`) sigue disponible sin app TVPik.

## API inbound TVPik (hub Webnu)

Base: `{TVPIK_API_URL}` (ej. `http://127.0.0.1:8001/api/v1`)

| Método | Ruta | Auth |
|--------|------|------|
| POST | `/integrations/webnu/hub/bootstrap` | `X-Webnu-Token` + app key |
| GET | `/integrations/webnu/screens` | Bearer TVPik + `X-Webnu-Token` |
| POST | `/integrations/webnu/screens` | idem — crear pantalla |
| PATCH | `/integrations/webnu/screens/{id}` | idem — renombrar |
| DELETE | `/integrations/webnu/screens/{id}` | idem — eliminar |
| POST | `/integrations/webnu/screens/{id}/pair` | idem — emparejar TV |
| POST | `/integrations/webnu/publish` | idem — publicar carta |

Implementación Webnu: [`TvpikHubService`](../app/Services/Tvpik/TvpikHubService.php), [`TvpikApiClient`](../app/Services/Tvpik/TvpikApiClient.php).
